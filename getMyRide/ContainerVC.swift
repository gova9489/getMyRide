...
extension ContainerVC: CenterVCDelegate {
    func toggleLeftPanel() {
        ...
        let notAlreadyExoanded = (currentState != .leftPanelExpanded)
        if notAlreadyExoanded {
            addLeftPanelViewController()
        }
        animateLeftPanel(shouldExpand: notAlreadyExoanded)
    }
        ...
        for subview in self.centerController.view.subviews {
            if subview.tag == 25 {
                UIView.animate(withDuration: 0.2, animations: { 
                    subview.alpha = 0.0}, completion: { (finished) in
...
private extension UIStoryboard {
    ...
    class func leftViewController() -> MenuLeftPanelVC? {
